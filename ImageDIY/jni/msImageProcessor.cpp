...
void msImageProcessor::LAB2RGB(const float& lval, const float& aval, const float& bval, BYTE& sR, BYTE& sG, BYTE& sB)
{
	...
	sR = (0.5+R*255.0)>255?255:(0.5+R*255.0);//min(0.5 + R*255.0, 255);
	sG = (0.5+G*255.0)>255?255:(0.5+G*255.0);//min(0.5 + G*255.0, 255);
	sB =(0.5+B*255.0)>255?255:(0.5+B*255.0); //min(0.5 + B*255.0, 255);
}
